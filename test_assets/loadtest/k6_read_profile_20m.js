...
function hitList() {
  ...
  totalReq.add(1); latList.add(res.timings.duration);
...
function hitSemantic() {
  ...
  const res = http.post(`${baseUrl}/api/resources/semantic-search`, JSON.stringify({ query: q }), {
    headers: { ...authHeaders, 'Content-Type': 'application/json' }, tags: { endpoint: 'semantic' },
  });
  totalReq.add(1); latSemantic.add(res.timings.duration);
...
function hitGraph() {
  ...
  const res = http.get(`${baseUrl}/api/rag/workspaces/${workspaceId}/graph?scope=${scope}&include_format_nodes=true`, {
    headers: authHeaders, tags: { endpoint: 'graph', scope },
  });
  totalReq.add(1); latGraph.add(res.timings.duration);
  const ok = check(res, { 'graph status 200': (r) => r.status === 200 });
  errGraph.add(!ok);
}
function hitPreview() {
  const res = http.get(`${baseUrl}/api/resources/${previewResourceId}/preview`, {
    headers: authHeaders, tags: { endpoint: 'preview' },
  });
  totalReq.add(1); latPreview.add(res.timings.duration);
